...
Bei starker Diffusionshemmung findet die Reaktion deahalb nur in der Randzone statt. 
...
Fluid mit linearer Geschwindigkeit $u$, Dichte $\rho$ und Viskosität $\nu$ umströmt Katalysatorkorn.\\
...
\[ \fbox{$ \displaystyle r_obs=\beta\cdot\left(c_b-c_s\right)=k_s\cdot c^n_s\qquad\left[\frac{mol}{m^2s}\right]$} \]
\begin{table}[H]
\begin{tabular}{ll}
...
$\beta_{Pr}>0$: exotherme Reaktion mit $\eta>1$ (wenn {\it stark} exotherm, selten) und $\eta<1$\\
...
Mit der dimensionslosen Größe {\sc Ha}:
