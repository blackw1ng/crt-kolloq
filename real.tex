...
\item Axiale Dispersion (Rückvermischung)
\item Ausbildung von Kanälen
\item Toträume
\item Kurzschluss-Ströme
\end{itemize}
Untersuchungen zum Strömungsverhalten können selten am tatsächlichen Reaktor durchgeführt werden.
In den meisten Fällen wird hierfür ein Modellreaktor im Labormaßstab herangezogen.
\subsection{Verweilzeitverteilung}
In realen Reaktoren entspricht die tatsächliche Verweilzeit (VWZ) nicht der hydrodynamischen VWZ $\tau=\frac{\dot V}{V}$, 
sondern folgt einer entsprechenden Altersverteilung.\\
VWZ-Verteilung am Reaktorausgang ({\sc exit}) durch Stoßmarkierung mit der definierten Menge $n_0$:
\[ \fbox{$\displaystyle E(t)=\frac{\dot n}{n_0}=\frac{\dot Vc(t)}{\int_0^{\infty}\dot Vc(t)dt}$} \]
\[ \int_0^{\infty}E(t)dt=1 \]
Das Integral der VWZ-Verteilung zwischen $t=0$ und $t=t_1$ entspricht dem Anteil an Molekülen, die kürzer als $t_1$ im Reaktor verweilen.
\begin{figure}[H]
\begin{center}
\includegraphics[width=80mm]{images/vwz-vert.eps}
\end{center}
\end{figure}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=80mm]{images/vwz-summe.eps}
\end{center}
\end{figure}
...
Dispersion ist ein {\it makroskopischer} Prozess, verursacht durch Varianzen des Strömungsverhaltens (Turbulenz, etc.).\\
...
$Bo\rightarrow 0$: VWZ-Kurve nähert sich dem idealen STR\\
...
Für $Bo>50$ gilt $Bo\approx2N$.
\subsubsection{Gemischte Modelle}
In manchen Fällen reichen die beiden Modelle mit jeweils nur einem Parameter nicht aus, um das beobachtete VWZ-Verhalten zu beschreiben.
In diesen Fällen werden gemischte Modelle (Kombination idealer Reaktoren) angewandt.
