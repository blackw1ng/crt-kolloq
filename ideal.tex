...
\item keine axiale vermischeung; z.B. PFTR
\end{itemize}
...
+\underbrace{div\left(D^e_igradc_i\right)}_{\mbox{Dispersion}}+\underbrace{\sum_j\nu_{ij}r_j}_{\mbox{Reaktion}}$}\qquad\left[\frac{kmol}{m^3s}\right]  \]
Wärmebilanz:\\
\[ \fbox{$\displaystyle \underbrace{\frac{\partial\left(\rho c_pT\right)}{\partial t}}_{\mbox{Akk.}}=
\underbrace{-div\left(\rho c_pT\bar u\right)}_{\mbox{Konvektion}}+\underbrace{div\left(\lambda^egradT\right)}_{\mbox{eff. Wärmeltg.}}+
...
\[ \underbrace{divD^e_i\underbrace{grad\underbrace{c_i}_{\mbox{scalar}}}_{\mbox{Vektor}}}_{\mbox{Skalar}}=\underbrace{D^e_i\left(\frac{\partial^2c_i}{\partial x^2}+ \frac{\partial^2c_i}{\partial y^2}+\frac{\partial^2c_i}{\partial z^2}\right)}_{\mbox{2. {\sc Fick}'sches Gesetz}} \]
...
Massenbilanz:
\[ \fbox{$\displaystyle \underbrace{\frac{dc_i}{dt}}_{\mbox{Akkumulation}}=\underbrace{\frac{\dot V_0}{V}c_{i0}}_{\mbox{Zustrom}}-\underbrace{\frac{\dot V_e}{V}c_{i}}_{\mbox{Abstrom}}+\underbrace{R_i}_{\mbox{Reaktion}}$}  \]
Wärmebilanz:
...
Ein Reaktand wird im Reaktor vorgelegt, der andere wird kontinuierlcih hinzugegeben.\\
...
Es entfallen die Terme für Konvektion.\\
Massenbilanz:\\
...
\begin{figure}[H]
\includegraphics[width=87mm]{images/reaktionszeit.eps}
\end{figure}
...
Wärmebilanz:
\[ \fbox{$\underbrace{\frac{dT}{dz}}_{\mbox{Konvektion}}=\frac{1}{u\rho c_p}\underbrace{\left(\sum_{j}\left(-\Delta H_j\right)r_j\right)}_{\mbox{Wärmequelle/-senke}}-\underbrace{k_W\frac{A}{V}\left(T-T_K\right)}_{\mbox{Wärmetransport}}$} \]
...
Kleine Temperaturabweichung verursacht Zünden oder Erlöschen $\rightarrow$ stabiler Betriebspunkt stellt sich ein.
\end{itemize}
